          ...
          child: Column(
            ...
            children: [
              ...
              Align(
                ...
                child: Text(
                  "${AppStrings.verifyAccountBody.tr} ${signUpProvider.email}",
                  style: AppTxtStyles.getAppRegularFont(
              ...
              Directionality(
                  ...
                  child: PinCodeFields(
                    ...
                    onComplete: (output) {
                      if(output.isEmpty)
                        {
                          signUpProvider.isValidForm2 = false;
                        }
                      else
                        {
                          signUpProvider.isValidForm2 = true;
                        }
                      signUpProvider.userOTP = output;
                    },
                  )),
              const SizedBox(
                height: 10,
              ...
              Align(
                ...
                child: InkWell(
                  onTap: () {
                    signUpProvider.requestResendOtp().then((response) {
                      if (response.apiStatus!) {
                        showSnackBar(
                            context: context,
                            msg: AppStrings.otpSentSuccessfully.tr);
                      } else {
                        showSnackBar(
                            context: context,
                            msg: AppStrings.somethingWentWrong.tr);
                      }
                    });
                  },
              ...
              TextButton(
                style: getDefaultBtnStyle(signUpProvider.isValidForm2),
                onPressed: () {
                  signUpProvider.validateOtp().then((response) {
                    if (response.apiStatus!) {
                      signUpProvider.disposeData();
                      Get.to(SignInScreen());
                    } else {
                      showSnackBar(
                ...
                child: Align(
                  alignment: Alignment.center,
                  child: Text(AppStrings.verifyCreateAccount.tr,
                      textAlign: TextAlign.center,
                      style: getBtnTxtStyle(signUpProvider.isValidForm2)),
                ),
              ),
            ],
          ),
